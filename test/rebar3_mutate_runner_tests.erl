...
baseline_passing_test() ->
    {_Forms, File} = compile_test_module(),
    Result = rebar3_mutate_runner:run_baseline(eunit, mutate_test_target, 10000),
    ?assertMatch({ok, _Duration}, Result),
    {ok, Duration} = Result,
    ?assert(Duration >= 0),
    cleanup(mutate_test_target, File).

baseline_failing_test() ->
    {_Forms, File} = compile_failing_module(),
    Result = rebar3_mutate_runner:run_baseline(eunit, mutate_test_failing, 10000),
    ?assertMatch({error, _}, Result),
    cleanup(mutate_test_failing, File).
...
compile_failing_module() ->
    Dir = tmp_dir(),
    filelib:ensure_dir(filename:join(Dir, "dummy")),
    File = filename:join(Dir, "mutate_test_failing.erl"),
    Src =
        "-module(mutate_test_failing).\n"
        "-export([add/2]).\n"
        "-ifdef(TEST).\n"
        "-include_lib(\"eunit/include/eunit.hrl\").\n"
        ...
        "add(A, B) -> A + B.\n",
    ok = file:write_file(File, Src),
    {ok, mutate_test_failing, Bin} = compile:file(File, [binary, {d, 'TEST'}]),
    {module, mutate_test_failing} = code:load_binary(mutate_test_failing, File, Bin),
    {ok, _Mod, Forms} = rebar3_mutate_ast:parse_file(File, []),
    {Forms, File}.
